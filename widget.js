...
function entrar(){var codigo=document.getElementById('lw-codigo').value.trim().toUpperCase(),erroEl=document.getElementById('lw-erro'),btn=document.getElementById('lw-entrar');erroEl.textContent='';if(!codigo){erroEl.textContent='Pon tu codigo';return;}btn.disabled=true;btn.textContent='Entrando...';fetch(API+'/auth/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({access_code:codigo})}).then(function(res){return res.json().then(function(data){if(!res.ok)throw new Error(data.error||'Codigo invalido');return data;});}).then(function(data){tok=data.token;if(data.user&&data.user.dark_mode)aplicarDark(true);document.getElementById('lw-login').style.display='none';var chat=document.getElementById('lw-chat');chat.style.display='flex';chat.style.flexDirection='column';return fetch(API+'/historico',{headers:{'Authorization':'Bearer '+tok}}).then(function(r){return r.json();}).then(function(hist){if(hist.mensagens&&hist.mensagens.length){hist.mensagens.forEach(function(m){var tipo=m.role==='assistant'?'lucia':'user';var conteudo=typeof m.content==='string'?m.content:(m.content&&m.content[0]&&m.content[0].text)||'';var ts=m.timestamp||m.created_at||Date.now();if(conteudo)addMsgHist(tipo,conteudo,ts);});var sep=document.createElement('div');sep.className='lw-sep';sep.textContent='— hoy —';sep.style.marginTop='16px';document.getElementById('lw-msgs').appendChild(sep);ultimoDia=new Date().toDateString();}}).catch(function(){}).then(function(){var d=dig();return fetch(API+'/chat',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+tok},body:JSON.stringify({mensagem:'[abertura_sesion]'})}).then(function(r){return r.json();}).then(function(rd){d.remove();if(rd.texto){addMsg('lucia',rd.texto);setTok(rd.tokens_remaining||'--');}document.getElementById('lw-input').focus();});});}).catch(function(err){erroEl.textContent=err.message;btn.disabled=false;btn.textContent='Entrar';});}
